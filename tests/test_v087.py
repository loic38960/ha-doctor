...
from sharing_v087 import build_share_report, HARD_BYTES
...
class V087Tests(unittest.TestCase):
    def _report(self):
        ...
        for i in range(15):
            ...
            findings.append({
                ...
                "domain": "automations",
                "priority": priority,
                "summary": "Long diagnostic summary " * 40,
                "recommendation": "Long recommendation " * 40,
                "examples": [
                    {
            ...
            actions.append({
                ...
                "first_check": {
                    "title": "Open the relevant configuration",
                    "detail": "Compare the relevant configuration in detail " * 20,
                },
                "temporal": {
                    "status": "persistent",
                    "occurrences": 20,
        ...
        return {
            ...
            "condition_semantics": {
                ...
                "unproven_pairs": [
                    ...
                    {"entity_id": "switch.boost", "automations": ["D", "E"], "target_kind": "actuator"},
                    {"entity_id": "switch.pump", "automations": ["F", "G"], "target_kind": "actuator"},
    ...
    def test_controller_review_counts_use_entities_and_pairs_correctly(self):
        ...
        action = next(
            item for item in report["action_plan"]["items"]
            if item["source_id"] == "HD-AUTO-003"
        )
        self.assertIn("4 entité(s)", action["evidence"][0]["text"])
        self.assertIn("7 paire(s)", action["evidence"][0]["text"])
        self.assertNotIn("7 entité(s)", action["evidence"][0]["text"])
        self.assertEqual(report["diagnostic_summary"]["source"], "final_correlated_action_plan_v087")

    def test_hardening_updates_delivery_metadata_without_touching_score(self):
        report = self._report()
        before = report["scores"]["global"]
        hardened = harden_report_v087(report)
        self.assertEqual(hardened["scores"]["global"], before)
        self.assertEqual(hardened["version"], "0.8.7")
        self.assertEqual(hardened["score_meta"]["hardening_version"], "0.8.7")
        self.assertEqual(hardened["score_meta"]["share_report_model"], "assistant_share_report_v2")
        self.assertEqual(hardened["report_schema"]["version"], "ha-doctor-report/0.8.7")
        self.assertIn("assistant_share_report_v2", hardened["report_schema"]["capabilities"])

    def test_share_v2_is_hard_bounded_and_preserves_diagnostic_ids(self):
        report = harden_report_v087(self._report())
